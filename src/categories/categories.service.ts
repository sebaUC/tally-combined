  ...
  async create(
    ...
    if ((count ?? 0) >= 50) {
      return {
        ...
        message: 'Máximo 50 categorías permitidas.',
      };
    }
    ...
    if (dto.parentId) {
      ...
      if (parent.parent_id) {
        return {
          error: 'MAX_DEPTH',
          message:
            'No se puede crear una subcategoría dentro de otra subcategoría.',
        };
      }
    }
  ...
  async remove(userId: string, categoryId: string, force = false) {
    ...
    if ((childCount ?? 0) > 0) {
      ...
      if (childCats?.length) {
        ...
        await this.supabase.from('categories').delete().in('id', childIds);
      }
    }
