...
export class ResponseBuilderService {
    ...
    switch (tool) {
      ...
      case 'manage_categories': {
        ...
        if (op === 'list') {
          ...
          return (
            `📂 <b>Tus categorías:</b>\n` +
            categories
              .map((c) => `${c.icon || '📌'} ${this.escapeHtml(c.name)}`)
              .join('\n')
          );
        }
      ...
      case 'ask_balance': {
        ...
        const categoryLabel = filter?.category
          ? ` en ${this.escapeHtml(filter.category)}`
          : '';
        const typeLabel =
          filter?.type === 'expense'
            ? ' (gastos)'
        ...
        if (filter) {
          ...
          if (filter.type !== 'income' && spent > 0) {
            lines.push(
              `💸 Gastos${categoryLabel} ${periodLabel}: <b>$${this.formatCLP(spent)}</b>`,
            );
          }
          if (filter.type !== 'expense' && income > 0) {
            lines.push(
              `💰 Ingresos${categoryLabel} ${periodLabel}: <b>$${this.formatCLP(income)}</b>`,
            );
          }
          if (lines.length === 0) {
        ...
        if (data?.activeBudget?.amount && !filter) {
          const remaining =
            data.activeBudget.remaining ?? data.activeBudget.amount - spent;
        ...
        if (op === 'list') {
          ...
          return txs
            .map((tx, i) => {
              ...
              const displayName =
                tx.name && tx.name !== tx.category
                  ? this.escapeHtml(tx.name)
                  : '';
              const catLabel = tx.category
                ? `${icon} ${this.escapeHtml(tx.category)}`
                : '';
              // Show: "1. $4.000 — 💊 Salud · Pasta de Dientes · 26/3"
              const parts = [
                `${i + 1}. <b>$${this.formatCLP(tx.amount)}</b> — ${catLabel}`,
              ];
    ...
    switch (tool) {
      ...
      case 'manage_categories': {
        if (
          ...
          data?.operation === 'create_and_register'
        ) {
          const catName = data?.category?.name ?? data?.name;
          if (catName) {
  ...
  private getCategoryIcon(category?: string): string {
    if (!category) return '💳';
    return pickCategoryEmoji(category);
  }
}
