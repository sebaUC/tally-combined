...
export async function getBalance(
  ...
  // 2. Date range
  const { start, end, label } = getDateRange(period, args.start_date, args.end_date);
  ...
  // Note: income has no category_id — don't filter by category
  ...
  if (includeBreakdown && expenses?.length) {
    ...
    for (const tx of expenses) {
      ...
      byCategory.set(catName, (byCategory.get(catName) || 0) + Number(tx.amount));
    }
  ...
  switch (period) {
    ...
    default: { // month
      ...
      return {
        ...
        label: now.toLocaleDateString('es-CL', { month: 'long', year: 'numeric', timeZone: 'America/Santiago' }),
      };
    }
  }
}
